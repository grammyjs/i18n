...
const initOptions = {
  ...
  resources: {
    ...
    ru: {
      translation: {
        greeting: "<b>I can't speak Russian. Hi {{first_name}}!</b>",
        cart_one:
          "No Russian: {{first_name}}, You have one pizza in your cart.",
        cart_other:
          "No Russian: {{first_name}}, You have {{number}} pizza's in your cart.",
...
bot.command(
  ...
  async (ctx) =>
    await ctx.reply(
      ctx.i18n.t("greeting", { first_name: ctx.from?.first_name }),
      { parse_mode: "HTML" }
    )
);
...
// Set locale to `it` (Fallback Language will be used)
bot.command("it", async (ctx) => {
  ctx.i18n.changeLanguage("it");
  return await ctx.reply(
    ctx.i18n.t("greeting", { first_name: ctx.from?.first_name }),
    { parse_mode: "HTML" }
  );
...
bot.command("add", async (ctx) => {
  ...
  const message = ctx.i18n.t("cart", {
    count: ctx.session.pizzaCount,
    number: ctx.session.pizzaCount,
