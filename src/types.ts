...
export interface I18nContextFlavor {
  ...
  i18n: {
    ...
    /**
     * Sets the specified locale to be used for future translations. Effect
     * lasts only for the duration of current update and is not preserved.
     * Could be used to change the translation locale in the middle of update
     * processing (e.g. when user changes the language).
     * @param locale Locale ID to set.
     */
    useLocale(locale: LocaleId): void;
    /**
     * You can manually trigger additional locale negotiation by calling this
     * method. This could be useful if locale negotiation conditions has changed
...
export interface I18nConfig<C extends Context = Context> {
  /**
   * A locale ID to use by default. This is used when locale negotiator and
   * session (if enabled) returns an empty result. The default value is: "_en_".
   */
